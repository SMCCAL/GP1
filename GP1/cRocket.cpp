...
void cRocket::update(float deltaTime)
{
	...
	if (m_InputMgr->isKeyDown(VK_SPACE))
	{
		...
		theRocketBullets[numBullets]->setTextureDimensions(theGameTextures[4]->getTWidth()/16, theGameTextures[4]->getTHeight()/16);
		...
		m_SoundMgr->getSnd("Chicken")->playAudio(AL_TRUE);
	}

	//Edge Barrier at top, bottom, and sides of screen
	if (spritePos2D.y + (textureHeight/2) > 640)
	{
	...
	if (spritePos2D.x + (textureWidth / 2) > 960)
	{
